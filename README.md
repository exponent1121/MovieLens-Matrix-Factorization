# MovieLens-100k Matrix Factorization Recommender from Scratch
...
이를 보완하기 위해 **Global Bias(전체 평균), User Bias(유저 편향), Item Bias(아이템 편향)**를 수식에 통합하여 예측 정밀도를 극대화했습니다. <br>
...
> **Result Summary:** 5개 데이터셋의 평균 RMSE 결과 **0.912738**을 기록하며, 목표 베이스라인이었던 **0.9130**을 성공적으로 돌파.
...
* **해결:** 모델의 숨통을 조이지 않으면서도 과적합을 강하게 억제할 수 있도록 $L_2$ Regularization Term(정규화 패널티)의 가중치를 **`reg=0.10`**으로 상향 조정. <br>
